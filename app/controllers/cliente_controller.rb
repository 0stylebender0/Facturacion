class ClienteController < ApplicationController
  ...
  def terminar_compra
    ...
    if carrito.empty?
      redirect_to cliente_factura_path, alert: "El carrito está vacío. Agrega productos antes de terminar la compra." and return
    end
    cliente_id = session[:cliente_id]
    unless cliente_id
      redirect_to cliente_login_form_path, alert: "Debes iniciar sesión para terminar la compra." and return
    end
  ...
  def comprar_producto
    ...
    redirect_to cliente_factura_path, notice: "Stock actualizado para #{producto.Nombre}"
  end

  def limpiar_carrito
    session[:carrito] = []
    redirect_to cliente_factura_path, notice: "Carrito limpiado correctamente."
  end

  def require_admin
    unless session[:cliente_id]
      redirect_to cliente_login_form_path, alert: "Por favor, inicia sesión como administrador"
    end
  end
